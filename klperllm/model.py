...
class GPTConfig:
    block_size: int = 1024
    vocab_size: int = 50304  # GPT-2 BPE vocab (50257) padded up to 64
    n_layer: int = 8
    n_head: int = 8
    n_embd: int = 512
    dropout: float = 0.0
...
class LayerNorm(nn.Module):
    """LayerNorm with an optional bias (PyTorch's built-in doesn't support bias=False)."""
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        # scaled init for residual projections, as per GPT-2 paper
